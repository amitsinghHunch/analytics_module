...
class FlutterNativeAd extends FlutterAd {
  ...
  void onNativeAdLoaded(@NonNull NativeAd nativeAd) {
    if (nativeTemplateStyle != null) {
      ...
      Log.d("TemplateView", "onNativeAdLoadedt: " + templateView.getMeasuredHeight());
    } else {
    ...
    String truncatedTextH = nativeAd.getHeadline();
    //remove \n from truncatedTextH
    //make truncatedTextH of 90 chars
    if (truncatedTextH!=null){
      truncatedTextH = truncatedTextH.replace("\n", " ");
      truncatedTextH = truncatedTextH.substring(0, Math.min(truncatedTextH.length(), 90));
    }
    else {
      truncatedTextH = "";
    }

    String truncatedTextB = nativeAd.getBody();
    //remove \n from truncatedTextB
    if (truncatedTextB!=null){
      truncatedTextB = truncatedTextB.replace("\n", " ");
      truncatedTextB = truncatedTextB.substring(0, Math.min(truncatedTextB.length(), 90));
    }
    else {
      truncatedTextB = "";
    }
    assert nativeAdView != null;
    float h = nativeAdView.getMeasuredHeight();
    float h2= dpFromPx(nativeAdView.getContext(),h);


    manager.onAdLoaded(adId, nativeAd.getResponseInfo(),h2,truncatedTextH,truncatedTextB);
  }
